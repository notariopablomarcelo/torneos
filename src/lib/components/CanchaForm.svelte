<script lang="ts">
	...
	import { canchaInputSchema, type CanchaInput } from '$lib/types/sede';
	import TextField from './TextField.svelte';

	type Props = {
		initial: CanchaInput;
		submitLabel?: string;
		onSubmit: (data: CanchaInput) => Promise<void>;
		onCancel?: () => void;
		onTest?: () => CanchaInput;
	...
	async function handleSubmit(e: Event) {
		...
		const parsed = canchaInputSchema.safeParse({ nombre });
		if (!parsed.success) {
	...
	function err(campo: 'nombre'): string | null {
...
<form onsubmit={handleSubmit} class="space-y-4">
	<TextField id="cancha-nombre" label="Nombre" bind:value={nombre} error={err('nombre')} />
